...
int insert_symbol(char *name, unsigned kind, unsigned type, 
  ...
  symbol_table[index].types= (unsigned*)malloc(MAXSIZE*sizeof(unsigned));
  int i;
  for (i=0; i<MAXSIZE; i++){
     symbol_table[index].types[i]=NO_TYPE;
  }
...
void set_types(int index, unsigned type, int i){
   if(index > -1 && index < SYMBOL_TABLE_LENGTH)
      symbol_table[index].types[i] = type;
}

unsigned get_types(int index, int i){
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].types[i];
  return NO_ATR;
}
...
void print_symtab(void) {
  ...
  int i,j;
  printf("\n\nSYMBOL TABLE\n");
  printf("\n       name           kind   type  atr1   atr2       types");
  printf("\n-- ---------------- -------- ----  -----  -----     ------");
  for(i = 0; i < first_empty; i++) {
    ...
    int j;
    for(j=0; j<MAXSIZE; j++)
	printf("%u ", symbol_table[i].types[j]);
  }
  printf("\n\n");
}
